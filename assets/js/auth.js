/* NightmareFTW — accounts + cross-device settings sync.
   ...
   - Email + password: a Cloudflare Worker + D1 account. The browser keeps only a
     signed session token; settings are one JSON blob per account. Password reset
     uses one-time RECOVERY CODES (shown once at sign-up) — no email needed.
   - Sign in with GitHub: the original OAuth flow, settings kept in a private gist.
   Profile (nickname + avatar) lives in synced nftw:profile:* keys, so it follows
   you across devices on either account. Vanilla JS, no build. */
(function () {
  "use strict";
  ...
  function openModal() {
    closeModal();
    var mode = "login"; // login | signup | recover
      ...
      wrap.innerHTML =
        '<div class="nftw-modal" role="dialog" aria-modal="true">' +
          ...
          '<form class="nftw-form">' +
            ...
            (mode === "login" ? '<button type="button" class="nftw-forgot">Forgot password?</button>' : '<p class="nftw-hint">At least 8 characters. Your settings and game progress sync to this account.</p>') +
          '</form>' +
    ...
    function drawRecover() {
      wrap.innerHTML =
        '<div class="nftw-modal" role="dialog" aria-modal="true">' +
          '<button class="nftw-modal-x" aria-label="Close">×</button>' +
          '<h3 class="nftw-h">Recover your account</h3>' +
          '<p class="nftw-hint">Enter your email, one of your recovery codes, and a new password. Each code works once.</p>' +
          '<form class="nftw-form">' +
            '<label>Email<input type="email" name="email" autocomplete="email" required></label>' +
            '<label>Recovery code<input type="text" name="code" placeholder="ABCD-EFGH-JKMN" autocomplete="off" required></label>' +
            '<label>New password<input type="password" name="password" autocomplete="new-password" minlength="8" required></label>' +
            '<p class="nftw-err" hidden></p>' +
            '<button type="submit" class="btn nftw-submit">Reset password</button>' +
            '<button type="button" class="nftw-forgot nftw-back">← Back to sign in</button>' +
          '</form>' +
        '</div>';
      wrap.querySelector(".nftw-modal-x").onclick = closeModal;
      wrap.onclick = function (e) { if (e.target === wrap) closeModal(); };
    ...
    function showCodes(codes) {
      ...
      wrap.innerHTML =
        '<div class="nftw-modal" role="dialog" aria-modal="true">' +
          '<h3 class="nftw-h">Save your recovery codes</h3>' +
          '<p class="nftw-hint">If you forget your password, any one of these resets it — no email needed. They\'re shown <b>only once</b>, so keep them somewhere safe.</p>' +
          ...
          '<button type="button" class="btn nftw-done">I\'ve saved them — continue</button>' +
        '</div>';
      wrap.onclick = null; // force acknowledge; no click-outside close
      wrap.querySelector(".nftw-copy").onclick = function () { try { navigator.clipboard.writeText(text); this.textContent = "Copied"; } catch (e) {} };
    ...
    async function onRecover(ev) {
      ...
      try {
        ...
        if (!r.ok || !r.data.token) { btn.disabled = false; return showErr(r.data.error === "weak_password" ? "New password must be at least 8 characters." : "That email + recovery code isn't valid."); }
        ls.setItem(CTOKEN, r.data.token); ls.setItem(CEMAIL, r.data.email || email);
        closeModal(); render(); await cloudPull();
      } catch (e) { btn.disabled = false; showErr("Network error — please try again."); }
    }
    draw();
  }
  function errMsg(code, mode) {
    return ({ email_taken: "That email already has an account — try signing in.", bad_credentials: "Wrong email or password.", weak_password: "Password must be at least 8 characters.", invalid_email: "That doesn't look like a valid email." })[code] || (mode === "signup" ? "Couldn't create the account." : "Couldn't sign in.");
  }
  ...
  function render() {
    ...
    if (signedIn()) {
      ...
      btn.title = "Your profile & account";
  ...
  // Expose a tiny API for the profile page (same-origin, same closure not shared,
  // so profile.js talks to the Worker directly; this just covers sign-out + sync).
  window.NFTWAuth = {
    ...
    pushNow: cloudPush, render: render,
  };
