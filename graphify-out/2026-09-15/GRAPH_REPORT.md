# Graph Report - NightmareFTW.github.io  (2026-09-07)

## Corpus Check
- 190 files · ~1,509,329 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1607 nodes · 2520 edges · 230 communities (115 shown, 115 thin omitted)
- Extraction: 94% EXTRACTED · 6% INFERRED · 0% AMBIGUOUS · INFERRED: 156 edges (avg confidence: 0.67)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `14e35aaf`
- Run `git rev-parse HEAD` and compare to check if the graph is stale.
- Run `graphify update .` after code changes (no API cost).

## Community Hubs (Navigation)
- update-items.js
- update-hsr-tierlist.js
- Game8
- animals.js
- Game8 (data source)
- The Outlast Trials Game Hub
- auth.js
- update-recipes.js
- items.js
- update-news.js
- gifts.js
- index.js
- update-outlast.js
- builds.js
- loadout.js
- builds.js
- update-codes.js
- friendship.js
- builds.js
- update-demonologist.js
- update-nte-teams.js
- missables.js
- missables.js
- builds.js
- missables.js
- update-ffw-builds.js
- builds.js
- Expedition 33 Missables Checklist
- tier-list.js
- update-ffw-maps.js
- update-hsr-builds.js
- update-hsr-teams.js
- evidence.js
- murdoku.js
- update-marvel-snap.js
- star-path.js
- gear-score.js
- meta-builds.js
- trials.js
- t
- update-hsr-banners.js
- Disney Dreamlight Valley (game hub)
- maps.js
- nodes.js
- builds.js
- ghost-evidence.js
- drops.js
- home.js
- demons.js
- speed-tuning.js
- event-calendar.js
- warp-calendar.js
- enemies.js
- update-drops.js
- update-marvel-snap-codes.js
- recipes.js
- cycles.js
- checklist.js
- i18n.js
- equipment.js
- equipment.js
- game.js
- damage.js
- builds.js
- Epic Seven (game hub)
- update-ravenswatch.js
- Update Marvel Snap Cards
- cursed-possessions.js
- Hades — Dreamlight Valley character portrait
- data.js
- Anna — Dreamlight Valley character portrait
- Ariel — Dreamlight Valley character portrait
- Aurora — Dreamlight Valley character portrait
- Belle — Dreamlight Valley character portrait
- Bruni the Fire Spirit — Dreamlight Valley character portrait
- Buzz Lightyear — Dreamlight Valley character portrait
- Cruella de Vil — Dreamlight Valley character portrait
- Daisy Duck — Dreamlight Valley character portrait
- Donald Duck — Dreamlight Valley character portrait
- Elsa — Dreamlight Valley character portrait
- EVE — Dreamlight Valley character portrait
- Flynn Rider — Dreamlight Valley character portrait
- Gaston — Dreamlight Valley character portrait
- Goofy — Dreamlight Valley character portrait
- Jack Skellington — Dreamlight Valley character portrait
- Jafar — Dreamlight Valley character portrait
- Kristoff — Dreamlight Valley character portrait
- Maleficent — Dreamlight Valley character portrait
- Maui — Dreamlight Valley character portrait
- Merida — Dreamlight Valley character portrait
- Merlin — Dreamlight Valley character portrait
- Mickey Mouse — Dreamlight Valley character portrait
- Mike Wazowski — Dreamlight Valley character portrait
- Minnie Mouse — Dreamlight Valley character portrait
- Mirabel — Dreamlight Valley character portrait
- Moana — Dreamlight Valley character portrait
- Mother Gothel — Dreamlight Valley character portrait
- Mulan — Dreamlight Valley character portrait
- Nala — Dreamlight Valley character portrait
- Olaf — Dreamlight Valley character portrait
- Oswald the Lucky Rabbit — Dreamlight Valley character portrait
- Pocahontas — Dreamlight Valley character portrait
- Prince Eric — Dreamlight Valley character portrait
- Rapunzel — Dreamlight Valley character portrait
- Remy — Dreamlight Valley character portrait
- Scar — Dreamlight Valley character portrait
- Scrooge McDuck — Dreamlight Valley character portrait
- Simba — Dreamlight Valley character portrait
- Snow White — Dreamlight Valley character portrait
- Stitch — Dreamlight Valley character portrait
- Sulley — Dreamlight Valley character portrait
- The Beast — Dreamlight Valley character portrait
- The Fairy Godmother — Dreamlight Valley character portrait
- The Forgotten — Dreamlight Valley character portrait (spiked crown emblem)
- Tiana — Dreamlight Valley character portrait
- Tigger — Dreamlight Valley character portrait
- Tinker Bell — Dreamlight Valley character portrait
- Ursula — Dreamlight Valley character portrait
- Vanellope — Dreamlight Valley character portrait
- WALL-E — Dreamlight Valley character portrait
- Woody — Dreamlight Valley character portrait
- Cyberpunk 2077 — game header banner
- Demonologist — game header banner
- Disney Dreamlight Valley — game header banner
- Elden Ring — game header banner
- Epic Seven (New Era) — game header banner
- Clair Obscur: Expedition 33 — game header banner
- Far Far West — game header banner
- Final Fantasy XIV Online — game header banner
- God of War Ragnarok — game header banner
- Honkai: Star Rail (The Lethe Below the Living) — game header banner
- Marvel Snap — game header banner
- Neverness to Everness — game header banner
- The Outlast Trials — game header banner
- Phasmophobia — game header banner
- Warframe — game header banner
- Adler — NTE character portrait
- Aurelia — NTE character portrait
- Baicang — NTE character portrait
- Chiz — NTE character portrait
- Daffodil — NTE character portrait
- Edgar — NTE character portrait
- Esper Zero — NTE character portrait
- Fadia — NTE character portrait
- Haniel — NTE character portrait
- Hathor — NTE character portrait
- Hotori — NTE character portrait
- Jiuyuan — NTE character portrait
- Lacrimosa — NTE character portrait
- Mint — NTE character portrait
- Nanally — NTE character portrait
- Sakiri — NTE character portrait
- Skia — NTE character portrait
- Crucifix — Phasmophobia equipment icon
- D.O.T.S Projector — Phasmophobia equipment icon
- EMF Reader — Phasmophobia equipment icon
- Firelight — Phasmophobia equipment icon
- Flashlight — Phasmophobia equipment icon
- Head Gear (Head-Mounted Camera) — Phasmophobia equipment icon
- Igniter (Ghost Huntin' Safety Matches) — Phasmophobia equipment icon
- Incense (Smudge Sticks) — Phasmophobia equipment icon
- Medication (Insane Away) — Phasmophobia equipment icon
- Motion Sensor — Phasmophobia equipment icon
- Parabolic Microphone — Phasmophobia equipment icon
- Photo Camera — Phasmophobia equipment icon
- Salt — Phasmophobia equipment icon
- Sound Sensor — Phasmophobia equipment icon
- Spirit Box — Phasmophobia equipment icon
- Thermometer — Phasmophobia equipment icon
- Tripod — Phasmophobia equipment icon
- UV Flashlight — Phasmophobia equipment icon
- Video Camera — Phasmophobia equipment icon
- Writing Book — Phasmophobia Equipment Icon
- Home Page Screenshot — Dark Hacker-Theme Game Hub Grid
- NTE Tier List Screenshot
- Phasmophobia Equipment Page Screenshot
- Warframe Drops Tracker Screenshot
- Site Favicon / Logo Mark
- Demonologist (game hub)
- ddv-catalogue.js
- update-animals.js
- update-starpath.js
- stamp-assets.js
- talents.js
- update-the-other-side.js
- evidence.js
- objects.js
- profile.js
- update-vampire-survivors.js
- heroes.js
- ghosts.js
- equipment.js
- NightmareFTW auth + sync Worker
- keep.js
- vs-curated-guides.js
- update-palworld.js
- deck-builder.js
- character.js
- update-steam-free-games.js
- dungeons.js
- achievements.js
- steam-alerts.js
- characters.js
- pal.js
- NightmareFTW · Gaming Tools Hub
- builds.js
- breeding.js
- update-farever.js
- weapons.js
- update-hsr-events.js
- pals.js
- passives.js
- add-csp.js
- add-analytics.js
- Demonologist Wiki
- updates-carousel.js
- Update Far Far West Builds
- Update Game News
- Cyberpunk 2077 Game Page
- Update Warframe Drop Table
- Update Neverness to Everness Teams
- Update Dreamlight Valley Data
- tier-list.js
- weapon.js
- enemies.js
- vs-xref.js
- arcana.js
- passive.js
- enemy.js

## God Nodes (most connected - your core abstractions)
1. `t()` - 37 edges
2. `fetch()` - 20 edges
3. `run()` - 17 edges
4. `clean()` - 12 edges
5. `officialName()` - 11 edges
6. `rowsFromPage()` - 11 edges
7. `detailFor()` - 11 edges
8. `clean()` - 10 edges
9. `stripWiki()` - 10 edges
10. `buildCard()` - 9 edges

## Surprising Connections (you probably didn't know these)
- `parseCompanions()` --indirect_call--> `t()`  [INFERRED]
  scripts/update-animals.js → games/warframe/worldstate.js
- `ddv()` --indirect_call--> `t()`  [INFERRED]
  scripts/update-codes.js → games/warframe/worldstate.js
- `game8Codes()` --indirect_call--> `t()`  [INFERRED]
  scripts/update-codes.js → games/warframe/worldstate.js
- `run()` --indirect_call--> `t()`  [INFERRED]
  scripts/update-hsr-tierlist.js → games/warframe/worldstate.js
- `rowsFromPage()` --indirect_call--> `t()`  [INFERRED]
  scripts/update-items.js → games/warframe/worldstate.js

## Import Cycles
- None detected.

## Hyperedges (group relationships)
- **Scheduled scrape-and-commit workflow pattern** — _github_workflows_update_ddv_update_dreamlight_valley_data, _github_workflows_update_demonologist_update_demonologist_data, _github_workflows_update_drops_update_warframe_drop_table, _github_workflows_update_ffw_builds_update_far_far_west_builds, _github_workflows_update_ffw_maps_update_far_far_west_maps, _github_workflows_update_hsr_banners_update_honkai_star_rail_warp_calendar, _github_workflows_update_hsr_builds_update_honkai_star_rail_character_builds, _github_workflows_update_hsr_events_update_honkai_star_rail_event_calendar, _github_workflows_update_hsr_teams_update_honkai_star_rail_meta_teams, _github_workflows_update_hsr_tierlist_update_honkai_star_rail_tier_list, _github_workflows_update_marvel_snap_update_marvel_snap_cards, _github_workflows_update_news_update_game_news, _github_workflows_update_nte_teams_update_neverness_to_everness_teams [INFERRED 0.85]
- **Cross-game Meta Builds tool pattern** — games_elden_ring_builds_meta_builds, games_expedition_33_builds_meta_builds, games_far_far_west_builds_meta_builds [INFERRED 0.85]
- **Cross-game Missables Checklist tool pattern (on-device ticks)** — games_elden_ring_missables_missables_checklist, games_expedition_33_missables_missables_checklist [INFERRED 0.85]
- **Dreamlight Valley character roster** — assets_img_ddv_anna_anna, assets_img_ddv_ariel_ariel, assets_img_ddv_aurora_aurora, assets_img_ddv_belle_belle, assets_img_ddv_bruni_the_fire_spirit_bruni_the_fire_spirit, assets_img_ddv_buzz_lightyear_buzz_lightyear, assets_img_ddv_cruella_de_vil_cruella_de_vil, assets_img_ddv_daisy_duck_daisy_duck [INFERRED 0.75]
- **Dreamlight Valley character roster** — assets_img_ddv_donald_duck_donald_duck, assets_img_ddv_elsa_elsa, assets_img_ddv_eve_eve, assets_img_ddv_flynn_rider_flynn_rider, assets_img_ddv_gaston_gaston, assets_img_ddv_goofy_goofy, assets_img_ddv_hades_hades, assets_img_ddv_hercules_hercules [INFERRED 0.75]
- **Disney Dreamlight Valley character roster** — assets_img_ddv_jack_skellington_jack_skellington, assets_img_ddv_jafar_jafar, assets_img_ddv_kristoff_kristoff, assets_img_ddv_maleficent_maleficent, assets_img_ddv_maui_maui, assets_img_ddv_merida_merida, assets_img_ddv_merlin_merlin, assets_img_ddv_mickey_mouse_mickey_mouse [INFERRED 0.75]
- **Disney Dreamlight Valley character roster** — assets_img_ddv_mike_wazowski_mike_wazowski, assets_img_ddv_minnie_mouse_minnie_mouse, assets_img_ddv_mirabel_mirabel, assets_img_ddv_moana_moana, assets_img_ddv_mother_gothel_mother_gothel, assets_img_ddv_mulan_mulan, assets_img_ddv_nala_nala, assets_img_ddv_olaf_olaf [INFERRED 0.75]
- **Disney Dreamlight Valley character roster** — assets_img_ddv_oswald_oswald, assets_img_ddv_pocahontas_pocahontas, assets_img_ddv_prince_eric_prince_eric, assets_img_ddv_rapunzel_rapunzel, assets_img_ddv_remy_remy, assets_img_ddv_scar_scar, assets_img_ddv_scrooge_mcduck_scrooge_mcduck, assets_img_ddv_simba_simba [INFERRED 0.75]
- **Disney Dreamlight Valley character roster** — assets_img_ddv_snow_white_snow_white, assets_img_ddv_stitch_stitch, assets_img_ddv_sulley_sulley, assets_img_ddv_the_beast_the_beast, assets_img_ddv_the_fairy_godmother_the_fairy_godmother, assets_img_ddv_the_forgotten_the_forgotten, assets_img_ddv_tiana_tiana, assets_img_ddv_tigger_tigger [INFERRED 0.75]
- **NTE character roster** — assets_img_nte_daffodil_daffodil, assets_img_nte_edgar_edgar, assets_img_nte_esper_zero_esper_zero, assets_img_nte_fadia_fadia, assets_img_nte_haniel_haniel, assets_img_nte_hathor_hathor, assets_img_nte_hotori_hotori, assets_img_nte_jiuyuan_jiuyuan [INFERRED 0.75]
- **Phasmophobia equipment set** — assets_img_phasmo_firelight_firelight, assets_img_phasmo_flashlight_flashlight, assets_img_phasmo_headgear_headgear, assets_img_phasmo_igniter_igniter, assets_img_phasmo_incense_incense, assets_img_phasmo_medication_medication, assets_img_phasmo_motionsensor_motion_sensor, assets_img_phasmo_parabolic_parabolic_microphone [INFERRED 0.75]
- **Phasmophobia equipment set** — assets_img_phasmo_photocamera_photo_camera, assets_img_phasmo_salt_salt, assets_img_phasmo_soundsensor_sound_sensor, assets_img_phasmo_spiritbox_spirit_box, assets_img_phasmo_thermometer_thermometer, assets_img_phasmo_tripod_tripod, assets_img_phasmo_uv_uv_flashlight, assets_img_phasmo_videocamera_video_camera [INFERRED 0.75]

## Communities (230 total, 115 thin omitted)

### Community 0 - "update-items.js"
Cohesion: 0.14
Nodes (19): imageUrls(), BIOMES, biomesIn(), dlcOf(), fs, isLimited(), LOC, LOC_PAIRS (+11 more)

### Community 1 - "update-hsr-tierlist.js"
Cohesion: 0.27
Nodes (9): cleanName(), decode(), { execFileSync }, fs, getHtml(), OUT, path, run() (+1 more)

### Community 2 - "Game8"
Cohesion: 0.48
Nodes (7): Game8, Update Honkai Star Rail Warp Calendar, Update Honkai Star Rail Character Builds, Update Honkai Star Rail Event Calendar, Update Honkai Star Rail Meta Teams, Update Honkai Star Rail Tier List, Honkai: Star Rail

### Community 3 - "animals.js"
Cohesion: 0.14
Nodes (27): approachOf(), BIOME_ORDER, biomeRank(), buildControls(), companionCard(), critterCard(), DAY3, DAYS_FULL (+19 more)

### Community 4 - "Game8 (data source)"
Cohesion: 0.11
Nodes (27): FFXIV Daily / Weekly Checklist, Final Fantasy XIV (game hub), FFXIV Gathering Node Timer, God of War Ragnarök Meta Builds, God of War Ragnarök (game hub), God of War Ragnarök Missables Checklist, PowerPyx (data source), Honkai Star Rail Character Builds (+19 more)

### Community 5 - "The Outlast Trials Game Hub"
Cohesion: 0.11
Nodes (25): Game8 (data source), Tier List & Builds (Neverness to Everness), Loadout Composition (1 Rig + 1 Tool + 1 Skill + 1 Medicine amp), Recommended Builds (The Outlast Trials), Enemies & Counters (The Outlast Trials), Outlast Wiki (data source), The Outlast Trials Game Hub, Loadout Builder (The Outlast Trials) (+17 more)

### Community 6 - "auth.js"
Cohesion: 0.20
Nodes (20): api(), applyBlob(), closeModal(), cloudPull(), cloudPush(), cloudSignOut(), collect(), fetchGithubUser() (+12 more)

### Community 7 - "update-recipes.js"
Cohesion: 0.16
Nodes (20): ADJ, SMALL, translateName(), tword(), WORD, clean(), dlcOfCollection(), fs (+12 more)

### Community 8 - "items.js"
Cohesion: 0.20
Nodes (19): biomeLabel(), buildDropdowns(), buildTabs(), CAT_ORDER, DLC_CLASS, els, esc(), from() (+11 more)

### Community 9 - "update-news.js"
Cohesion: 0.18
Nodes (21): articleMeta(), bbcodeToHtml(), decode(), decodeEntities(), escAttr(), fetchT(), fs, googleNews() (+13 more)

### Community 10 - "gifts.js"
Cohesion: 0.16
Nodes (19): charCard(), detail, flashCap(), fons(), giftKind(), giftRow(), GIFTS, ORDER (+11 more)

### Community 11 - "index.js"
Cohesion: 0.16
Nodes (29): auth(), b64u(), checkPassword(), checkRateLimit(), clientIp(), corsHeaders(), dec, enc (+21 more)

### Community 12 - "update-outlast.js"
Cohesion: 0.18
Nodes (19): api(), bullets(), dewiki(), ENV_ORDER, envTrials(), field(), fileUrls(), fs (+11 more)

### Community 13 - "builds.js"
Cohesion: 0.22
Nodes (17): attrBars(), ATTRS, buildCard(), buildFilters(), colorForTree(), cyberDiagram(), esc(), I (+9 more)

### Community 14 - "loadout.js"
Cohesion: 0.18
Nodes (14): AMP_GROUPS, build, builds, chips(), encode(), optButtons(), persist(), renderSaved() (+6 more)

### Community 15 - "builds.js"
Cohesion: 0.27
Nodes (15): buildCard(), buildFilters(), esc(), gearDiagram(), I, open, primaryStat(), render() (+7 more)

### Community 16 - "update-codes.js"
Cohesion: 0.28
Nodes (19): altCodes(), clean(), ddv(), ddvPcgamesn(), DIR, epic7(), ffw(), firstCode() (+11 more)

### Community 17 - "friendship.js"
Cohesion: 0.22
Nodes (14): DLC, dlcClass(), grid, levels, lvl(), render(), renderSummary(), ROLES (+6 more)

### Community 18 - "builds.js"
Cohesion: 0.25
Nodes (14): attrBars(), ATTRS, characterScreen(), elBuilds, elTabs, elTeam, esc(), render() (+6 more)

### Community 19 - "update-demonologist.js"
Cohesion: 0.22
Nodes (14): clean(), demonsWithEvidence(), EVID, { execFileSync }, fs, get(), ib(), OUT (+6 more)

### Community 20 - "update-nte-teams.js"
Cohesion: 0.18
Nodes (14): CHARS, cleanLabel(), data, { execFileSync }, FIX, fs, get(), norm() (+6 more)

### Community 21 - "missables.js"
Cohesion: 0.25
Nodes (13): allItems(), done, esc(), FLAG_LABEL, itemId(), progressEl, render(), renderIntro() (+5 more)

### Community 22 - "missables.js"
Cohesion: 0.25
Nodes (13): allItems(), done, esc(), FLAG_LABEL, itemId(), progressEl, render(), renderIntro() (+5 more)

### Community 23 - "builds.js"
Cohesion: 0.33
Nodes (14): buildCard(), BY_ID, esc(), icon(), jokerRow(), openDetail(), RARITY, render() (+6 more)

### Community 24 - "missables.js"
Cohesion: 0.25
Nodes (13): allItems(), done, esc(), FLAG_LABEL, itemId(), progressEl, render(), renderIntro() (+5 more)

### Community 25 - "update-ffw-builds.js"
Cohesion: 0.24
Nodes (13): balancedObj(), clean(), detailOf(), { execFileSync }, fs, getHtml(), jokerName(), listBuilds() (+5 more)

### Community 26 - "builds.js"
Cohesion: 0.29
Nodes (12): ATTR_COL, buildCard(), buildFilters(), col(), DMG_STATS, esc(), itemCard(), open (+4 more)

### Community 27 - "Expedition 33 Missables Checklist"
Cohesion: 0.18
Nodes (13): Community Elden Ring API, Elden Ring Meta Builds, Elden Ring (game hub), Elden Ring Missables Checklist, Expedition 33 Meta Builds, Clair Obscur: Expedition 33 (game hub), Game8, Expedition 33 Missables Checklist (+5 more)

### Community 28 - "tier-list.js"
Cohesion: 0.24
Nodes (12): CHARACTERS, detail, esc(), portrait(), PRYDWEN_TEAMS, render(), root, showBuild() (+4 more)

### Community 29 - "update-ffw-maps.js"
Cohesion: 0.24
Nodes (11): balancedArray(), { execFileSync }, fs, getHtml(), OUT, path, regionFrom(), rscOf() (+3 more)

### Community 30 - "update-hsr-builds.js"
Cohesion: 0.21
Nodes (17): cleanCell(), cleanName(), decode(), { execFileSync }, fs, getHtml(), LIST_URLS, OUT (+9 more)

### Community 31 - "update-hsr-teams.js"
Cohesion: 0.27
Nodes (11): cleanName(), decode(), ELEMENTS, { execFileSync }, fs, getHtml(), OUT, parseTeams() (+3 more)

### Community 32 - "evidence.js"
Cohesion: 0.24
Nodes (10): demonMatches(), DEMONS, esc(), EVIDENCE, labelOf(), NEXT, render(), state (+2 more)

### Community 33 - "murdoku.js"
Cohesion: 0.32
Nodes (15): accuse(), act(), blockedBy(), drawMap(), edgeLine(), edgeWindow(), fixtureLayer(), hint() (+7 more)

### Community 34 - "update-marvel-snap.js"
Cohesion: 0.27
Nodes (10): decode(), { execFileSync }, fs, getJson(), norm(), OUT, path, run() (+2 more)

### Community 35 - "star-path.js"
Cohesion: 0.31
Nodes (10): build(), checks, dutyCard(), esc(), progress(), root, ROUTINE, save() (+2 more)

### Community 36 - "gear-score.js"
Cohesion: 0.27
Nodes (9): addRow(), breakdownEl, calc(), optionsHtml, rowsEl, scoreEl, SUBSTATS, tierEl (+1 more)

### Community 37 - "meta-builds.js"
Cohesion: 0.42
Nodes (9): dpsBlock(), elBar, esc(), member(), render(), renderTabs(), roleClass(), root (+1 more)

### Community 38 - "trials.js"
Cohesion: 0.42
Nodes (8): closeModal(), esc(), mapCard(), matches(), missionPanel(), openMap(), render(), root

### Community 39 - "t"
Cohesion: 0.06
Nodes (52): eorzea(), etEl, fmt(), JOB_LABEL, listEl, NODES, nodeState(), render() (+44 more)

### Community 40 - "update-hsr-banners.js"
Cohesion: 0.27
Nodes (9): decode(), { execFileSync }, fs, getHtml(), iso(), MON, OUT, path (+1 more)

### Community 41 - "Disney Dreamlight Valley (game hub)"
Cohesion: 0.25
Nodes (9): DDV Animal Guide, Dreamlight Valley Wiki, DDV Friendship Tracker, Disney Dreamlight Valley (game hub), DDV Items Database, Crystal Dreams (recipe source), Nintendo Life, DDV Recipe Browser (+1 more)

### Community 42 - "maps.js"
Cohesion: 0.33
Nodes (8): active, catOf(), CATS, esc(), renderRegion(), renderTabs(), root, tabsEl

### Community 43 - "nodes.js"
Cohesion: 0.29
Nodes (15): arcanaCard(), buildControls(), buildTabs(), cardFor(), els, esc(), hrefFor(), itemsFor() (+7 more)

### Community 44 - "builds.js"
Cohesion: 0.53
Nodes (8): buildSection(), detail, esc(), portraitHTML(), render(), roleClass(), root, showBuild()

### Community 45 - "ghost-evidence.js"
Cohesion: 0.25
Nodes (8): EVIDENCE, ghostMatches(), GHOSTS, NEXT, render(), state, STATE_CLASS, STATE_LABEL

### Community 46 - "drops.js"
Cohesion: 0.31
Nodes (9): buildFacets(), els, esc(), matches(), render(), sel, TIER_ORDER, TYPE_LABEL (+1 more)

### Community 47 - "home.js"
Cohesion: 0.43
Nodes (6): cardHtml(), render(), saveOrderFromDom(), savePinnedFromDom(), sortList(), wireCards()

### Community 48 - "demons.js"
Cohesion: 0.43
Nodes (7): card(), esc(), evBar, labelOf(), render(), renderTabs(), root

### Community 49 - "speed-tuning.js"
Cohesion: 0.39
Nodes (7): colorFor(), renderRows(), renderSeq(), rowsEl, seqEl, simulate(), units

### Community 50 - "event-calendar.js"
Cohesion: 0.43
Nodes (7): esc(), fmt(), ms(), render(), root, STATUS, statusOf()

### Community 51 - "warp-calendar.js"
Cohesion: 0.39
Nodes (7): bannerCard(), esc(), render(), root, STATUS_LABEL, statusOf(), today

### Community 52 - "enemies.js"
Cohesion: 0.29
Nodes (7): CATS, DANGER_CLASS, ENEMIES, esc(), filters, render(), root

### Community 53 - "update-drops.js"
Cohesion: 0.32
Nodes (7): fs, getJson(), OUT, path, RARITIES, ri(), run()

### Community 54 - "update-marvel-snap-codes.js"
Cohesion: 0.32
Nodes (7): clean(), { execFileSync }, fs, getHtml(), OUT, path, run()

### Community 55 - "recipes.js"
Cohesion: 0.39
Nodes (7): buildStarFilter(), els, esc(), matches(), nm(), render(), starSel

### Community 56 - "cycles.js"
Cohesion: 0.36
Nodes (7): CYCLES, data, esc(), fetchAll(), fmt(), render(), root

### Community 57 - "checklist.js"
Cohesion: 0.53
Nodes (5): load(), nextReset(), periodKey(), render(), save()

### Community 58 - "i18n.js"
Cohesion: 0.60
Nodes (5): applyTranslation(), init(), injectSwitcher(), translateText(), walk()

### Community 59 - "equipment.js"
Cohesion: 0.53
Nodes (5): card(), esc(), labelOf(), render(), root

### Community 60 - "equipment.js"
Cohesion: 0.33
Nodes (4): CATS, EQUIPMENT, filtersEl, root

### Community 62 - "damage.js"
Cohesion: 0.80
Nodes (4): $(), calc(), fmt(), num()

### Community 63 - "builds.js"
Cohesion: 0.40
Nodes (3): BUILDS, root, SLOTS

### Community 64 - "Epic Seven (game hub)"
Cohesion: 0.50
Nodes (4): Epic Seven Damage / EHP Calculator, Epic Seven Gear Score Calculator, Epic Seven (game hub), Epic Seven Speed Tuning / Turn Order

### Community 65 - "update-ravenswatch.js"
Cohesion: 0.17
Nodes (25): api(), clean(), ENT, { execFileSync }, fileIn(), fs, get(), OUT (+17 more)

### Community 66 - "Update Marvel Snap Cards"
Cohesion: 0.67
Nodes (3): Marvel Snap, Marvel Snap Zone, Update Marvel Snap Cards

### Community 180 - "ddv-catalogue.js"
Cohesion: 0.13
Nodes (18): buildCatalogue(), fs, { officialName }, parseCatalogue(), path, { wikitext, imageUrls, sections, galleries }, fs, load() (+10 more)

### Community 181 - "update-animals.js"
Cohesion: 0.16
Nodes (26): clean(), fileAndName(), { getJson }, parseTable(), sections(), tables(), wikitext(), buildFoodMap() (+18 more)

### Community 182 - "update-starpath.js"
Cohesion: 0.23
Nodes (10): { execFileSync }, getJson(), getText(), clean(), fs, { getJson }, OUT, path (+2 more)

### Community 183 - "stamp-assets.js"
Cohesion: 0.15
Nodes (9): crypto, fs, hashes, i18n, missing, path, ROOT, SELF_INJECTED (+1 more)

### Community 184 - "talents.js"
Cohesion: 0.26
Nodes (11): ALL, card(), chips(), esc(), flatten(), heroBar, render(), renderBars() (+3 more)

### Community 185 - "update-the-other-side.js"
Cohesion: 0.26
Nodes (11): clean(), EV, evId(), { execFileSync }, fs, get(), OUT, path (+3 more)

### Community 186 - "evidence.js"
Cohesion: 0.24
Nodes (10): esc(), EVIDENCE, ghostMatches(), GHOSTS, labelOf(), NEXT, render(), state (+2 more)

### Community 187 - "objects.js"
Cohesion: 0.36
Nodes (9): card(), esc(), rarBar, rarClass(), RARITIES, rarRank(), render(), renderBar() (+1 more)

### Community 188 - "profile.js"
Cohesion: 0.51
Nodes (9): api(), loadRecoveryCount(), openEditor(), refreshHeader(), render(), showCodes(), syncNow(), toast() (+1 more)

### Community 189 - "update-vampire-survivors.js"
Cohesion: 0.10
Nodes (42): autoGuide(), balancedTemplate(), buildWeaponGraph(), chunk(), cleanEnemyName(), curatedGuides, enemyIntro(), { execFileSync } (+34 more)

### Community 190 - "heroes.js"
Cohesion: 0.54
Nodes (7): abilityHTML(), detail, esc(), portraitHTML(), render(), root, showHero()

### Community 191 - "ghosts.js"
Cohesion: 0.43
Nodes (7): card(), esc(), evBar, labelOf(), render(), renderTabs(), root

### Community 192 - "equipment.js"
Cohesion: 0.48
Nodes (6): card(), esc(), evOrder(), labelOf(), render(), root

### Community 193 - "NightmareFTW auth + sync Worker"
Cohesion: 0.40
Nodes (4): NightmareFTW auth + sync Worker, Notes / security, One-time deploy, Password recovery

### Community 195 - "vs-curated-guides.js"
Cohesion: 0.07
Nodes (26): avatarInfernasEn, avatarInfernasPt, bigTrouserEn, bigTrouserPt, chaosEn, chaosPt, cosmoPavoneEn, cosmoPavonePt (+18 more)

### Community 196 - "update-palworld.js"
Cohesion: 0.18
Nodes (21): balancedArr(), balancedObj(), clean(), detailFor(), elementsOf(), { execFileSync }, fs, getHtml() (+13 more)

### Community 197 - "deck-builder.js"
Cohesion: 0.23
Nodes (19): addCard(), artCard(), BY, CARDS, chips(), deck, deckEl, esc() (+11 more)

### Community 198 - "character.js"
Cohesion: 0.21
Nodes (17): allStepIds(), applyUnlockedFrom(), collectLeafItems(), esc(), refreshCounts(), render(), renderGuideItem(), renderNodes() (+9 more)

### Community 199 - "update-steam-free-games.js"
Cohesion: 0.20
Nodes (17): appDetails(), deadlineHint(), decode(), decodeEntities(), fetchGroupPosts(), fetchT(), fs, isCurrentlyFree() (+9 more)

### Community 200 - "dungeons.js"
Cohesion: 0.27
Nodes (14): allItems(), done, esc(), itemId(), itemImg(), itemMeta(), progressEl, render() (+6 more)

### Community 201 - "achievements.js"
Cohesion: 0.32
Nodes (14): buildControls(), confirmUnlockAll(), confirmUnlockGroup(), done, els, esc(), highlightFromQuery(), idOf() (+6 more)

### Community 202 - "steam-alerts.js"
Cohesion: 0.34
Nodes (13): closePanel(), fmtDate(), footer(), inject(), maybeNotify(), openPanel(), renderPanel(), start() (+5 more)

### Community 203 - "characters.js"
Cohesion: 0.32
Nodes (13): buildControls(), els, esc(), guideStepCount(), isRevealed(), isUnlockedC(), lockedCard(), opt() (+5 more)

### Community 204 - "pal.js"
Cohesion: 0.28
Nodes (12): buildSection(), EFFECT_LABEL, effectLabel(), esc(), fmtEffect(), passivesPool(), render(), root (+4 more)

### Community 205 - "NightmareFTW · Gaming Tools Hub"
Cohesion: 0.15
Nodes (12): A few in action, Accounts & sync, Adding a game or tool, Auto-updating data, Credits, NightmareFTW · Gaming Tools Hub, Play, Project structure (+4 more)

### Community 206 - "builds.js"
Cohesion: 0.26
Nodes (10): buildCard(), buildFilters(), DIFF_LABEL, esc(), HAS_IMG, open, render(), root (+2 more)

### Community 207 - "breeding.js"
Cohesion: 0.33
Nodes (10): closestByRank(), compute(), els, esc(), overrideKey(), pickResult(), renderPicked(), resultEl (+2 more)

### Community 208 - "update-farever.js"
Cohesion: 0.25
Nodes (10): clean(), decodeEntities(), { execFileSync }, fs, getHtml(), NAMED, OUT, parseDungeons() (+2 more)

### Community 209 - "weapons.js"
Cohesion: 0.29
Nodes (8): buildFilters(), card(), empty, esc(), grid, HAS_IMG, render(), WEAPON_ICON

### Community 210 - "update-hsr-events.js"
Cohesion: 0.29
Nodes (9): cellText(), decode(), { execFileSync }, fs, getHtml(), iso(), OUT, path (+1 more)

### Community 211 - "pals.js"
Cohesion: 0.42
Nodes (8): buildControls(), card(), ELEMENT_CLASS(), els, esc(), opt(), render(), WORK_LABEL

### Community 212 - "passives.js"
Cohesion: 0.36
Nodes (8): card(), EFFECT_LABEL, effectLabel(), els, esc(), fmtEffect(), RANK_LABEL, render()

### Community 213 - "add-csp.js"
Cohesion: 0.25
Nodes (6): ALLOW, fs, missingCharset, path, ROOT, SKIP_DIRS

### Community 214 - "add-analytics.js"
Cohesion: 0.29
Nodes (5): fs, missingBody, path, ROOT, SKIP_DIRS

### Community 215 - "Demonologist Wiki"
Cohesion: 0.60
Nodes (6): Update Demonologist Data, Demon Reference, Demonologist Wiki, Equipment Guide, Demon Evidence Checker, Demonologist

### Community 216 - "updates-carousel.js"
Cohesion: 0.70
Nodes (4): next(), prev(), restart(), show()

### Community 217 - "Update Far Far West Builds"
Cohesion: 0.67
Nodes (4): Update Far Far West Builds, wikily.gg, Update Far Far West Maps, Far Far West

### Community 218 - "Update Game News"
Cohesion: 0.67
Nodes (4): Google News, Steam Dev Posts, Update Game News, News Article Page

### Community 219 - "Cyberpunk 2077 Game Page"
Cohesion: 0.67
Nodes (4): Cyberpunk 2077 Meta Builds, Cyberpunk 2077 Game Page, Cyberpunk 2077, data.js Central Config

### Community 220 - "Update Warframe Drop Table"
Cohesion: 0.67
Nodes (3): Update Warframe Drop Table, WFCD Warframe Drop Tables, Warframe

### Community 221 - "Update Neverness to Everness Teams"
Cohesion: 0.67
Nodes (3): Prydwen, Update Neverness to Everness Teams, Neverness to Everness

### Community 223 - "tier-list.js"
Cohesion: 0.42
Nodes (11): buildSection(), compsFor(), detail, esc(), member(), portrait(), render(), roleClass() (+3 more)

### Community 224 - "weapon.js"
Cohesion: 0.36
Nodes (11): affectingArcanas(), effectsList(), esc(), evolutionSection(), recipeGroups(), render(), root, startingWeaponOf() (+3 more)

### Community 225 - "enemies.js"
Cohesion: 0.62
Nodes (6): buildControls(), card(), els, esc(), opt(), render()

### Community 226 - "vs-xref.js"
Cohesion: 0.53
Nodes (3): closePopup(), initXrefPopup(), openPopup()

### Community 227 - "arcana.js"
Cohesion: 0.53
Nodes (5): affectsHtml(), esc(), KIND_LABEL, render(), root

### Community 228 - "passive.js"
Cohesion: 0.67
Nodes (5): esc(), render(), root, statsTable(), usedByArcanas()

### Community 229 - "enemy.js"
Cohesion: 0.70
Nodes (4): esc(), render(), root, statsTable()

## Knowledge Gaps
- **584 isolated node(s):** `GAMES`, `SITE_UPDATES`, `ATTRS`, `SLOT_LAYOUT`, `I` (+579 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **115 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `t()` connect `t` to `update-hsr-tierlist.js`, `update-recipes.js`, `items.js`, `update-news.js`, `update-outlast.js`, `builds.js`, `builds.js`, `update-codes.js`, `builds.js`, `builds.js`, `tier-list.js`, `meta-builds.js`, `trials.js`, `update-animals.js`, `update-starpath.js`, `talents.js`, `update-vampire-survivors.js`, `update-ravenswatch.js`, `deck-builder.js`, `tier-list.js`?**
  _High betweenness centrality (0.100) - this node is a cross-community bridge._
- **Why does `run()` connect `update-vampire-survivors.js` to `t`?**
  _High betweenness centrality (0.015) - this node is a cross-community bridge._
- **Why does `rowsFromPage()` connect `update-animals.js` to `update-items.js`, `t`?**
  _High betweenness centrality (0.015) - this node is a cross-community bridge._
- **Are the 35 inferred relationships involving `t()` (e.g. with `buildCard()` and `selectTab()`) actually correct?**
  _`t()` has 35 INFERRED edges - model-reasoned connections that need verification._
- **Are the 2 inferred relationships involving `clean()` (e.g. with `ddv-fandom.js` and `parseCompanions()`) actually correct?**
  _`clean()` has 2 INFERRED edges - model-reasoned connections that need verification._
- **What connects `GAMES`, `SITE_UPDATES`, `ATTRS` to the rest of the system?**
  _584 weakly-connected nodes found - possible documentation gaps or missing edges._
- **Should `update-items.js` be split into smaller, more focused modules?**
  _Cohesion score 0.1368421052631579 - nodes in this community are weakly interconnected._