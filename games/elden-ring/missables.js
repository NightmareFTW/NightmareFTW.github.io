...
const KEY = "nftw:elden:missables";
...
const FLAG_LABEL = { "one-time": "Locks content", "post-game": "Opens later", "story": "Keep talking" };
...
(async function init() {
  try {
    DATA = await (await fetch(`../../data/elden-ring/missables.json?cb=${Date.now()}`)).json();
    render();
  } catch (e) {
