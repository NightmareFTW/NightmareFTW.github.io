/* Demonologist — Equipment Guide.
   Renders data/demonologist/data.json (scraped from the Demonologist Wiki).
   Tools with their function and which evidence they find; evidence tools first.
   Vanilla JS, no deps. */
...
function render() {
  ...
  // Evidence tools first, then utility; stable within each.
  list.sort((a, b) => (a.evidence ? 0 : 1) - (b.evidence ? 0 : 1));
