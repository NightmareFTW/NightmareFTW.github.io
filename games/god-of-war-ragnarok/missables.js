...
const FLAG_LABEL = { "one-time": "One-time area", "post-game": "Opens later", "story": "Story prompt" };

const root = document.getElementById("ms-root");
const progressEl = document.getElementById("ms-progress");

// Each item gets a stable id from its section + label.
const itemId = (sec, it) => `${slug(sec.title)}__${slug(it.label)}`;

function allItems() {
  return DATA.sections.flatMap((sec) => sec.items.map((it) => itemId(sec, it)));
}
...
(async function init() {
  try {
    DATA = await (await fetch(`../../data/god-of-war-ragnarok/missables.json?cb=${Date.now()}`)).json();
    render();
  } catch (e) {
